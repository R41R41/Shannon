...
describe('YouTube public search adapter', () => {
  it('uses search.list with a bounded query and no access token in the path', async () => {
    const search = vi.fn(async () => ({ items: [{ id: { videoId: video }, snippet: {
      title: 'New Game', channelTitle: 'Studio', publishedAt: new Date(NOW - 1000).toISOString(), description: 'fun',
    } }] }));
    ...
    expect(result).toEqual([{ videoId: video, title: 'New Game', channelTitle: 'Studio', publishedAt: NOW - 1000, fact: 'fun' }]);
  });
