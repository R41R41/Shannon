...
export const minebotAdapter: ChannelAdapter<MinebotNativeEvent> = {
  ...
  toEnvelope(event: MinebotNativeEvent): RequestEnvelope {
    ...
    return createEnvelope({
      ...
      conversationId: memoryKeys?.conversationId ?? `minecraft:${event.serverName ?? 'default'}:${event.senderName}`,
      threadId: memoryKeys?.threadId ?? `minecraft:${event.serverName ?? 'default'}`,
      text: event.message,
      tags,
      minecraft,
      metadata: {
