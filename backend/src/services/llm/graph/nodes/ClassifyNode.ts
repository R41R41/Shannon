...
- シンプルな挨拶・感想・雑談のみconversational（ツール不要）

# needsTools の判定基準（重要）
- 以下は必ず needsTools=true にする:
  - 最新・リアルタイムの情報が必要な質問（ニュース、天気、映画、イベント、価格、スポーツ結果等）
  - 「おすすめ」「最近の」「今度の」「〇月までに」など、モデルの訓練データに含まれない可能性がある情報
  - 「調べて」「教えて」「検索して」など明示的な調査依頼
  - 画像生成、Notion操作、Web取得などツール固有のタスク
- 以下のみ needsTools=false にする:
  - 挨拶（こんにちは、おはよう等）
  - 一般常識で答えられる質問（「日本の首都は？」等）
  - 感想・お礼・短い雑談
- 迷ったら needsTools=true にする（不要なツールはLLMが自分で省略できるが、必要なツールがないと回答不能）
