...
MANUAL_BLOCKERS = [
  'reviewed_uid_migration_and_login_evidence_required',
  'live_channel_and_spend_limits_required',
  'quiesced_database_backup_required_at_cutover',
  'native_artifact_and_full_build_validation_required',
...
def inspect(candidate,prod,automated_only=False):
 ...
 return {'ready':not blockers,'candidateHead':candidate_head,'productionHead':prod_head,'preservedBaseline':BASELINE,'automatedOnly':automated_only,'blockers':blockers}
if __name__=='__main__':
 parser=argparse.ArgumentParser();parser.add_argument('--candidate',required=True);parser.add_argument('--production',required=True)
 parser.add_argument('--automated-only',action='store_true',help='Skip manual cutover evidence blockers for CI/dev validation')
 a=parser.parse_args()
 try:result=inspect(a.candidate,a.production,a.automated_only);print(json.dumps(result,indent=2));raise SystemExit(0 if result['ready'] else 2)
