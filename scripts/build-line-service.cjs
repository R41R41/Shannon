...
async function main() {
  ...
  const schema = await build({ absWorkingDir: root, entryPoints: ['backend/src/services/line/databaseSchema.ts'], outfile: path.join(out,'database-schema.mjs'),
    bundle: true, platform: 'node', target: 'node22', format: 'esm', packages: 'external', metafile: true });
  ...
  fs.writeFileSync(path.join(out,'build-manifest.json'),JSON.stringify({inputs,imports},null,2)+'\n');
  console.log(JSON.stringify({built:true,inputs:inputs.length,imports}));
}
