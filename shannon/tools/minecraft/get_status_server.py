...
class GetStatusServerInput(BaseModel):
...
class GetStatusServerTool(BaseTool):
    # マイクラサーバーのステータスを確認するためのツール
    name = "get-status-server"
    description = "Tool to check the status of the Minecraft server."
    args_schema: Type[BaseModel] = GetStatusServerInput
    ...
    async def _arun(
        ...
        try:
            ...
            response = await U.send_request(
                endpoint='status_minecraft_server', data=data, destination="minecraft_server", request_type="post")
            if response:
