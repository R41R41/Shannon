import { RadarClientError, type RadarClient, type RadarErrorCode, type SourceInput, type SourcesSnapshot, type PreviewSnapshot } from './radarClient';

export interface RadarSnapshot {
  status: 'idle' | 'loading' | 'ready' | 'saving' | 'hidden' | 'expired' | 'error' | 'closed';
  data: { sources: SourcesSnapshot; preview: PreviewSnapshot } | null;
  error?: RadarErrorCode;
}
/** Per-mounted-session controller. No storage, polling, external collection or automatic mutation retry. */
export class RadarController {
  ...
  async load(minimumRevision = 0): Promise<void> {
    if (!this.allowed() || this.state.status === 'saving') return;
    ...
    try {
      ...
      if (!this.current(n)) return;
      if (sources.revision !== preview.revision || sources.revision < minimumRevision) throw new RadarClientError('conflict');
      ...
      const remaining = Math.min(60000, preview.validUntil - preview.servedAt) - (this.monotonicNow() - started);
      if (!Number.isFinite(remaining) || remaining <= 0) { this.clear('expired'); return; }
      this.deadline = this.monotonicNow() + remaining;
      this.set({ status: 'ready', data: { sources, preview } });
  ...
  private async mutate(id: string, source?: SourceInput): Promise<void> {
    ...
    this.clear('saving'); const n = this.generation; const abort = this.pending = new AbortController();
    try {
      const result = input ? await this.client.save(id, expectedRevision, input, abort.signal)
        : await this.client.revoke(id, expectedRevision, abort.signal);
      if (!this.current(n)) return;
      this.set({ status: 'idle', data: null });
      await this.load(result.revision); // Read back only. Never collect, publish or repeat the mutation.
    } catch (error) {
